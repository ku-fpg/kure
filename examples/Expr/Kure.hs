...
instance (ExtendPath c Int, AddDef c) => Walker c Universe where
   ...
   allR r = modExc (stackStrategyFailure "allR") $
            rewrite $ \ c -> \case
              GExpr e  -> GExpr <$> applyR allRexpr c e
              GCmd cm  -> GCmd  <$> applyR allRcmd c cm
     where
       allRexpr = addAllR (extractR r) (extractR r) <+> eseqAllR (extractR r) (extractR r) <+> idR
       allRcmd  = seqAllR (extractR r) (extractR r) <+> assignR (extractR r)

---------------------------------------------------------------------------

seqT :: (ExtendPath c Int, AddDef c, MonadThrow m) => Transform c m Cmd a1 -> Transform c m Cmd a2 -> (a1 -> a2 -> b) -> Transform c m Cmd b
seqT t1 t2 f = transform $ \ c -> \case
                                     Seq cm1 cm2 -> f <$> applyT t1 (c @@ 0) cm1 <*> applyT t2 (updateContextCmd cm1 c @@ 1) cm2
                                     _           -> throwM (nodeMismatch "Seq")

seqAllR :: (ExtendPath c Int, AddDef c, MonadThrow m) => Rewrite c m Cmd -> Rewrite c m Cmd -> Rewrite c m Cmd
...
assignT :: (ExtendPath c Int, MonadThrow m) => Transform c m Expr a -> (Name -> a -> b) -> Transform c m Cmd b
assignT t f = transform $ \ c -> \case
                                    Assign n e -> f n <$> applyT t (c @@ 0) e
                                    _          -> throwM (nodeMismatch "Assign")

assignR :: (ExtendPath c Int, MonadThrow m) => Rewrite c m Expr -> Rewrite c m Cmd
...
addT :: (ExtendPath c Int, MonadThrow m) => Transform c m Expr a1 -> Transform c m Expr a2 -> (a1 -> a2 -> b) -> Transform c m Expr b
addT t1 t2 f = transform $ \ c -> \case
                                     Add e1 e2 -> f <$> applyT t1 (c @@ 0) e1 <*> applyT t2 (c @@ 1) e2
                                     _         -> throwM (nodeMismatch "Add")

addAllR :: (ExtendPath c Int, MonadThrow m) => Rewrite c m Expr -> Rewrite c m Expr -> Rewrite c m Expr
...
eseqT :: (ExtendPath c Int, AddDef c, MonadThrow m) => Transform c m Cmd a1 -> Transform c m Expr a2 -> (a1 -> a2 -> b) -> Transform c m Expr b
eseqT t1 t2 f = transform $ \ c -> \case
                                      ESeq cm e1 -> f <$> applyT t1 (c @@ 0) cm <*> applyT t2 (updateContextCmd cm c @@ 1) e1
                                      _          -> throwM (nodeMismatch "ESeq")

eseqAllR :: (ExtendPath c Int, AddDef c, MonadThrow m) => Rewrite c m Cmd -> Rewrite c m Expr -> Rewrite c m Expr
