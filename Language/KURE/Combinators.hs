...
(<+) rr1 rr2 = translateTransparently $ \ dec e -> apply rr1 dec e `catchM` (\ _ -> apply rr2 dec e)
...
readerT fn = translateTransparently $ \ dec expA -> apply (fn expA) dec expA
...
getDecsT f = translateTransparently $ \ dec e -> apply (f dec) dec e

-- | change the @dec@'s for a scoped translation.
mapDecsT :: (Monoid dec,Monad m) => (dec -> dec) -> Translate m dec a r -> Translate m dec a r
mapDecsT f_env rr = translateTransparently $ \ env e -> apply rr (f_env env) e
...
acceptR fn = translateTransparently $ \ dec expA -> if fn expA 
				 	            then return expA
					            else fail "accept failed"

-- | identity rewrite.
idR :: (Monad m, Monoid dec) => Rewrite m dec exp
idR = rewriteTransparently $ \ dec e -> return e
...
wasId rr fn = translateTransparently $ \ dec e -> do
