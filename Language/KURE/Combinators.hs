...
(<+) :: MonadPlus m => Translate c m a b -> Translate c m a b -> Translate c m a b
...
contextT :: Pointed m => Translate c m a (c a)
contextT = translate point
...
tryR :: (Copointed c, Pointed m, MonadPlus m) => Rewrite c m a -> Rewrite c m a
tryR s = s <+ idR

-- | repeat a rewrite until it fails, then return the result before the failure.
repeatR :: (EndoFunctor c, Copointed c, Pointed m, MonadPlus m) => Rewrite c m a -> Rewrite c m a
...
extractT :: (InjectiveFunctor c, Term a) => Translate c m (Generic a) b -> Translate c m a b
...
promoteT  :: (InjectiveFunctor c, Monad m, Term a) => Translate c m a b -> Translate c m (Generic a) b
promoteT t = translate (maybe (fail "promote") (apply t) . retractC)

-- | 'extractR' converts a 'Rewrite' over a 'Generic' into a rewrite over a specific expression type.
extractR :: (InjectiveFunctor c, Monad m, Term a) => Rewrite c m (Generic a) -> Rewrite c m a
extractR r = extractT r >>= maybe (fail "extractR") return . retract
...
promoteR  :: (InjectiveFunctor c, Monad m, Term a) => Rewrite c m a -> Rewrite c m (Generic a)
